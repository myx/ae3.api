...
class HashMapPrimitiveIntExact<V> {
	 ...
	 */
	@SuppressWarnings("unchecked")
	HashMapPrimitiveIntExact() {
		this.loadFactor = HashMapPrimitiveIntExact.DEFAULT_LOAD_FACTOR;
		this.threshold = (int) (HashMapPrimitiveIntExact.DEFAULT_INITIAL_CAPACITY * HashMapPrimitiveIntExact.DEFAULT_LOAD_FACTOR);
		this.table = new Entry[HashMapPrimitiveIntExact.DEFAULT_INITIAL_CAPACITY];
	}
	 ...
	 */
	HashMapPrimitiveIntExact(final int initialCapacity) {
		this( initialCapacity, HashMapPrimitiveIntExact.DEFAULT_LOAD_FACTOR );
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	HashMapPrimitiveIntExact(int initialCapacity, final float loadFactor) {
		if (initialCapacity < 0) {
			throw new IllegalArgumentException( "Illegal initial capacity: " + initialCapacity );
		}
		if (initialCapacity > HashMapPrimitiveIntExact.MAXIMUM_CAPACITY) {
			initialCapacity = HashMapPrimitiveIntExact.MAXIMUM_CAPACITY;
		}
	...
	public final boolean containsKey(final int key) {
		final int i = key & this.table.length - 1;
	...
	public final V get(final int key) {
		for (Entry<V> e = this.table[key & this.table.length - 1]; e != null; e = e.next) {
			if (e.key == key) {
				return e.value;
			}
		}
	...
	private final void resize(final int newCapacity) {
		...
		if (oldCapacity == HashMapPrimitiveIntExact.MAXIMUM_CAPACITY) {
	...
	private void transfer(final Entry<V>[] newTable) {
		...
		for (int j = 0; j < src.length; j++) {
			...
			if (e != null) {
				...
				do {
					final Entry<V> next = e.next;
					final int i = e.key & newCapacity - 1;
					e.next = newTable[i];
					newTable[i] = e;
					e = next;
				} while (e != null);
