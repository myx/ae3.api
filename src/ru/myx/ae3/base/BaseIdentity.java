 ...
 */
@ReflectionDisable
public abstract class BaseIdentity<T> extends BaseAbstract<T> {
	...
	public static void setOwnProperty(
		...
		final String name = nameInstance.toString();
		...
		if (target.properties != null) {
			final BaseProperty existing = target.properties.find( nameInstance );
			if (existing != null) {
				if (!existing.isWritable( name ) && !existing.isDynamic( name )) {
					// ignore
					return;
				}
				target.properties = target.properties.delete( nameInstance );
			}
		}
		/**
		...
		target.properties = target.properties == null
				? new BasePropertyFilterPrimitive( nameInstance, property, name, writable, enumerable, dynamic )
				: target.properties.add( nameInstance, //
						new BasePropertyFilterPrimitive( property, name, writable, enumerable, dynamic ) );
	}
	
	BasePropertiesPrimitive	properties	= null;
	
	BaseObject<?>			prototype;
	
	/**
	 * 
	 * UNDEFINED replaced with BaseObject.PROTOTYPE
	 * 
	 * @param prototype
	 *            null to specify no prototype or prototype otherwise
	 */
	BaseIdentity(final BaseObject<?> prototype) {
